 ...
 * Calqué sur patient-service/notes-service security/SecurityConfigTest.
 *
 * @WebMvcTest SANS exclusion de la Security (contrairement à AssessmentControllerTest) :
...
class SecurityConfigTest {
    ...
    void should_return200_when_jwt_is_valid() throws Exception {
        when(assessmentService.assess(1L)).thenReturn(new AssessmentResponse(1L, "None", List.of()));

        String token = issueToken(testPrivateKey(), VALID_ISSUER, Instant.now(), Instant.now().plusSeconds(60));

        mockMvc.perform(get("/assessment/1").header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
